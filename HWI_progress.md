...
- [x] Users can delete imported wearable records and derived wearable context from Hydrion.
...
## Correctness and deletion sprint evidence (2026-09-22)

Branch: `fix/wearable-state-and-deletion-correctness`, base
`de06688f010a533e41f9e258b274d1329587ec8b`. No commit or remote operation.
The earlier local edits, including the unchecked iOS capability criterion, remain
preserved. Criteria text and total remain unchanged: **61**.
Before this sprint: **47 checked / 14 unchecked**.
After: **48 checked / 13 unchecked**. Only checkbox **line 94** changed.

Line 94 evidence: transactional recursive imported/derived deletion, rollback,
reopen and repeated deletion tests in
[test/encrypted_health_data_repository_test.dart](test/encrypted_health_data_repository_test.dart);
controller deletion failure/restart/manual-history tests; and the authorized
Infinix QA UI deleting three previously imported synthetic records, repeating
deletion, disconnecting and preserving both manual 150 ml entries after restart.
Derived rows were exercised in encrypted-repository tests, not claimed as
physically populated by this app. Broad physical Android/iOS and release criteria
remain unchecked; no Apple, Wear OS or OEM criterion is newly checked.

Validation: 82 focused tests and 782 full-suite tests passed; analyzer clean;
244 Dart files, zero format changes; EN/FR/ES 962/962, Android 24/24;
placeholder drift zero; secret scan, two-workflow validation and diff check passed.
Full suite included HealthKit/WidgetKit/native configuration tests, not fresh
Apple builds or physical tests. QA ARM64 build and in-place install succeeded.
See [edge_case.md section 21](edge_case.md#21-correctness-sprint-2026-09-22)
for artifact identity, physical observations, startup measurements and limitations.
