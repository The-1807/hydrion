...
| Challenge recommendations | Local deterministic ranking uses optional on-device routine preferences and current eligible cached weather. Adult BMI can only add a private weak secondary score after explicit water-rich-food interest. | User must open details and explicitly start; no medical or weight-loss claim. No meaningful signal displays no recommendation card. | Verify preference toggles, weather revocation, neutral explanations, the two-active-challenge limit, and dismissal behavior. |
