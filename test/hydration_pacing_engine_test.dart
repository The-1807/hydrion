...
void main() {
  ...
  group('missing or invalid schedule', () {
    test('both null reports unavailable and does not block progress fraction',
        () {
    ...
    test('does not prevent hydration tracking: progress fraction is always computed',
        () {
  ...
  group('ordinary daytime schedule (wake 07:00, sleep 23:00)', () {
    ...
    test('goal already reached reports goalReached even early in the window',
        () {
      final status = engine.calculate(
        wakeMinuteOfDay: 7 * 60,
        sleepMinuteOfDay: 23 * 60,
  ...
  group('overnight / shift-work schedule (wake 14:00, sleep 06:00)', () {
    ...
    test('mid-morning (10:00) after an overnight shift is outsideWakingWindow',
        () {
      final status = engine.calculate(
        wakeMinuteOfDay: 14 * 60,
        sleepMinuteOfDay: 6 * 60,
  ...
  group('safety invariants', () {
    test('pacing never influences dailyGoalMl: engine has no goal-mutation surface',
        () {
    ...
    test('respects a clinician-directed goal passed in as dailyGoalMl (goal '
    ...
    test('the waking window is a half-open interval: the sleep minute itself '
