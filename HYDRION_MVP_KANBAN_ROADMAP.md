...
| Transparent runtime asset replacement | Done | `lib/domain/ui_asset_manifest.dart`; `assets/UI_BETA/*.png`; `test/asset_registry_test.dart` | Boxed `hydrion-lifestyle-*.jpg` runtime art was replaced with transparent PNG assets; old runtime JPG lifestyle files were removed. Weather, challenge, and goals PNGs are now mapped where supplied. |
| Completed-profile demographic lock | Done | `lib/ui/screens/profile_screen.dart`; `test/runtime_ux_test.dart`; `test/startup_onboarding_test.dart` | First-time onboarding can set age/sex; completed profile edit preserves existing age/sex while keeping nickname, avatar, units, goal, and container editable. |
| Passive hydration rhythm rail | Done | `lib/ui/screens/home_screen.dart`; `test/runtime_ux_test.dart` | Home rail copy and cards are passive status cues driven by normal hydration progress; no button-like challenge/ritual action was added. |
